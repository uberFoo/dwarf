...
impl Console<'_> {
    fn new(object_file: &PathBuf) -> Console<'static> {
        ...
        let (repl_send, repl_recv) = start_tui_repl(interpreter);
    ...
    // 🚧 Factor out a Debugger struct I thnik.
    fn debugger_step_into(&mut self) {
        self.repl_send
            .send(DebuggerControl::StepInto)
            .expect("Failed to send step-into command");
    }

    fn debugger_run(&mut self) {
        self.repl_send
            .send(DebuggerControl::Run)
            .expect("Failed to send run command");
    }

    fn debugger_stop(&mut self) {
        self.repl_send
            .send(DebuggerControl::Stop)
            ...
            DebuggerStatus::StdOut(value) => {
                ...
                if value.contains('\n') {
                    app.write()
                        .console
                        .output
                        .extend(buffer.into_text().unwrap());
                    buffer.clear();
                } else {
    ...
    *lock.lock() = true;
    ...
    loop {
        ...
        if let Event::Key(key) = event {
            let mut app = app.write();
            match key.code {
                KeyCode::F(5) => app.console.debugger_run(),
                KeyCode::F(11) => app.console.debugger_step_into(),
