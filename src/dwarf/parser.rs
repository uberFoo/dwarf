...
impl DwarfParser {
    ...
    fn parse_attribute(&mut self) -> Option<Attribute> {
        ...
        let start = if let Some(tok) = self.match_(&[Token::Punct('[')]) {
            tok.1.start
        } else {
            debug!("exit");
            return None;
    ...
    fn parse_impl_block(&mut self) -> Option<Spanned<InnerItem>> {
        ...
        let open_brace = if let Some(tok) = self.match_(&[Token::Punct('{')]) {
            tok.1.start
        } else {
            let tok = self.previous().unwrap();
            let err = Simple::expected_input_found(
    ...
    fn parse_generic(&mut self) -> Result<Option<Spanned<Vec<Spanned<Type>>>>> {
        let start = if let Some(tok) = self.peek() {
            tok.1.start
        } else {
            return Ok(None);
    ...
    fn match_<'a>(&mut self, tokens: &'a [Token]) -> Option<Spanned<&'a Token>> {
        for tok in tokens {
            if self.check(tok) {
